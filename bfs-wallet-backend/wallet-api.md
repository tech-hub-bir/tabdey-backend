...
## Online Payment (BFS — System Wallet Credit)

These endpoints let a user pay for something via their bank account. The BFS flow is identical to wallet top-up (AR → AE → DR → AS), but on success the collected amount is credited to the **system wallet** (configured via `SYSTEM_WALLET_USER_ID` in `.env`). No user-level transaction record is created; RMA logs and the `wallet_transactions` entry are always kept.

### 5. Init Payment

**`POST /api/payment/init`**

Starts an online payment session. Returns the BFS transaction ID and available banks.
...
| `description` | string | No | What the payment is for (default: "Online payment") |
...
```json
{
  "ok": true,
  "data": {
    "orderNo": "PAY1745678901234567",
    "bfsTxnId": "BFS20240002",
    "bankList": [
      { "id": "1010", "name": "BANK OF BHUTAN LIMITED", "status": "A" },
      { "id": "1020", "name": "Bhutan National Bank Limited", "status": "A" }
    ]
...
Verifies the payer's bank account. Must be called after `/init`.
...
|---|---|---|---|
| `orderNo` | string | Yes | Order number from `/init` |
| `remitterBankId` | string | Yes | Bank ID chosen from `bankList` |
| `remitterAccNo` | string | Yes | Payer's bank account number |
...
Debits the payer's account via OTP and credits the system wallet on success.
...
|---|---|---|---|
| `orderNo` | string | Yes | Order number from `/init` |
| `otp` | string | Yes | OTP sent to the payer |
...
Fetches the current payment status from BFS. Also credits the system wallet if the status is `SUCCESS` and the wallet has not been credited yet.
