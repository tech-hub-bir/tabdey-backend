...
async function assertUserHasNotRated(business_id, user_id) {
  const existingRating = await prisma.food_ratings.findFirst({
    where: {
  ...
  if (existingRating) {
    throw makeError(
      "Thank you! You have already rated this restaurant. You can only rate once per restaurant.",
      409,
    );
  }
}
...
async function insertFoodRating({ business_id, user_id, rating, comment }) {
  try {
    ...
    // Create the rating
    await prisma.food_ratings.create({
      data: {
        business_id: bid,
        user_id: uid,
        rating: r,
...
async function fetchFoodRatings(business_id, { page = 1, limit = 20 } = {}) {
  try {
    ...
    const ratings = await prisma.food_ratings.findMany({
      where: { business_id: bid },
    ...
    console.error("fetchFoodRatings error:", error);
    throw error;
  }
}
