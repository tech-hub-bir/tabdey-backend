...
async function lookupFareFromTable(conn, { trip_category, from_location, to_location, trip_type }) {
  const from = safeStr(from_location);
  const to   = safeStr(to_location);
  ...
  if (trip_category === "inter_city") {
    ...
    const row = rows[0];
    if (!row) return null;
    const fareNu = trip_type === "pool"
      ? Number(row.share_fare   || 0)
      : Number(row.reserve_fare || 0);
    return { subtotal_cents: Math.round(fareNu * 100), is_share_allowed: null };
  }

  if (trip_category === "intra_city") {
    const rows = await qConn(
      conn,
    ...
    const row = rows[0];
    if (!row) return null;
    const isShareAllowed = Boolean(row.is_share);
    const fareNu = (trip_type === "pool" && isShareAllowed)
      ? Number(row.share_fare   || 0)
      : Number(row.reserve_fare || 0);
    return { subtotal_cents: Math.round(fareNu * 100), is_share_allowed: isShareAllowed };
  }
...
export async function computePlatformFeeAndGST(input) {
  ...
  const { trip_category, from_location, to_location } = input;

  return await withConn(async (conn) => {
    // Resolve subtotal from fare table when trip_category is provided;
    // otherwise fall back to the caller-supplied subtotal_cents.
    ...
    if (trip_category) {
      ...
      if (!fareRow) {
        throw Object.assign(
          new Error(`No fare found for route: ${from_location} → ${to_location}`),
          { statusCode: 404 },
        );
      }
    ...
    return {
      ...
      // present only when a fare table was used
      ...(trip_category != null && {
        trip_category,
        from_location,
        to_location,
